...
Build-time slide thumbnails for liebstoeckel decks. A headless browser renders each
...
build** — when `LIEBSTOECKEL_NO_THUMBS=1` is set or no Chromium is available. The
browser only launches at build time, never in dev.

Lower-level entry points:

```bash
bunx liebstoeckel-thumbnails dist/index.html            # --format webp --width 640 --quality 80 --scale 2
```

```ts
import { addThumbnailsToFile, captureThumbnails } from "@liebstoeckel/thumbnails";
...
1. The capturer injects `window.__LIEBSTOECKEL_CAPTURE__` before the deck boots, so the
...
2. It reads `__LIEBSTOECKEL_SLIDE_COUNT__`, then for each slide dispatches a
   `liebstoeckel:capture` event and waits for `__LIEBSTOECKEL_CAPTURE_READY__` to match
...
3. The browser yields a lossless PNG (Playwright only screenshots PNG/JPEG);
   **`Bun.Image`** (Bun's built-in codec — no `sharp`) transcodes it to a **WebP**
   ...
   them go into a `<script type="application/json" data-liebstoeckel-thumbnails>` block
   via `embedThumbnails` (re-running strips the prior block).

## Chromium

Resolves a binary from `executablePath` → `$LIEBSTOECKEL_CHROMIUM` → Playwright's
