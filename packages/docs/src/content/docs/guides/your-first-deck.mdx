...
`<Magic>` keeps an element *persistent across slides* by sharing a `layoutId`, so Motion FLIP-animates it from one position/size to the next — the signature present-it transition.

```tsx
import { Magic } from "@present-it/engine";
