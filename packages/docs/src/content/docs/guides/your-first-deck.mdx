...
Reveals replay in reverse when you go backwards. Pressing `←` hides the last
...
rather than advancing. The deck window shows the end card (`←` or `Esc` goes
back, `r` restarts); the presenter view simply stops advancing, and its Next
button is disabled to show why.
