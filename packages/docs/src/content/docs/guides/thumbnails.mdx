---
title: Thumbnails
description: Build-time slide thumbnails for the overview grid, captured headlessly, encoded as WebP, and embedded in the deck.
---
...
Both route through the same `withThumbnails(html, opts)` primitive, so the gate/capture/embed policy lives in one place. To get thumbnails when capture is being skipped, see [Chromium setup](/guides/chromium/).
