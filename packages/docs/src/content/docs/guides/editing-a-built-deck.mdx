...
asks you to trust it. In a terminal you confirm once; **non-interactively (CI, an agent,
piped input) add `--trust`** (or set `LIEBSTOECKEL_TRUST_BUILD=1`) or the build refuses,
fail-closed:

```bash
liebstoeckel build . --trust  # confirm an ejected deck non-interactively
```
