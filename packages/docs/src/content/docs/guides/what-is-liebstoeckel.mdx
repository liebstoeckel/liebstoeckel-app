...
<CardGrid>
  ...
  <Card title="Built for agents" icon="rocket">
    The CLI works for a coding agent as well as a person: `--json` output, a readable component registry, `build --check` for validation without an artifact, and `liebstoeckel skill install --global` to teach the agent the workflow.
  </Card>
