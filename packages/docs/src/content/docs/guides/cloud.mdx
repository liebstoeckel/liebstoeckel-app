...
build** — no CDN, fully self-contained). `liebstoeckel new` does the same for the org
default brand. Pass `--no-install` to write the files and install the font packages
yourself. A **Custom…** font carries no package — supply its `@font-face` yourself, or it
