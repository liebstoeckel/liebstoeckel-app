...
3. Start the dev server (instant HMR + React Fast Refresh):
   ...
   `bun run dev` is the deck's plain hot-reload server. For the annotation
   sidebar and the agent loop, run [`liebstoeckel dev`](/guides/dev-mode/)
   instead.
...
// Hot-module boundary: slide edits re-render into the same root during dev,
// so the deck keeps its position. Builds compile this down to a plain createRoot.
