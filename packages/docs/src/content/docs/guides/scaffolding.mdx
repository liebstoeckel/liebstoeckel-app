...
decisions; owned source keeps them yours. It also means an AI agent can do the
bending: the chart is a plain `.tsx` file in your deck, so you can point an agent at
it like any other code. (Engine-coupled primitives still ship as `@liebstoeckel/*`
packages. Only the composable pieces are scaffolded.)
</Aside>
