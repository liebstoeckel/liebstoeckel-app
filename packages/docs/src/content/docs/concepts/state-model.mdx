---
title: State model
description: "Deck state, its owners, transports, and authority, and how standalone and live mode differ."
---
...
- **Server restarts and moves:** a server that restarts, or hands the session to another server, closes sockets with a close code that says so (`4003` restarting, `4004` moved); clients reconnect at once and keep retrying about every half second while the session starts up again elsewhere, instead of backing off. `4006` means the talk ended: the client stops reconnecting and keeps its state for the end card.
- **Protocol version:** the client sends its live protocol version (`v` on the socket URL). A server that no longer supports it closes the socket with `4005` and a reason, and the page shows that a reload is needed instead of retrying forever. A client without a version counts as version 1.
- **Half-open detection:** the server sends a keepalive every 10 s and Bun pings idle sockets; the client runs a watchdog and reconnects if no frame arrives within `staleMs` (35 s by default), without waiting for a frozen server to finish the closing handshake; a connection attempt that has not opened within `connectTimeoutMs` (10 s) is retried the same way. A silently-dead (mobile-sleep / NAT-timeout) socket recovers instead of hanging.
