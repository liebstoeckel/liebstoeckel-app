...
- **Half-open detection:** the server sends a keepalive every 10 s and Bun pings idle sockets; the client runs a watchdog and reconnects if no frame arrives within `staleMs` (35 s by default). A silently-dead (mobile-sleep / NAT-timeout) socket recovers instead of hanging.
