...
| **Plugin shared state** (e.g. poll `votes`) | the plugin, via `pluginState()` | Yjs `plugin:<id>` map (CRDT) — or `plugin:<id>:<instance>` for a named instance ([ADR&nbsp;0033](https://github.com/…/docs/adr/0033-plugin-instances)) | **any** participant writes — incl. read-only viewers; presenter-only actions gated in plugin code | ephemeral (lost on server restart) |
