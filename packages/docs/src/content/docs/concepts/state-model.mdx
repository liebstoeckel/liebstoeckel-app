---
title: State model
description: "Every piece of deck state: its owner, transport, and authority, plus the one decision (standalone vs. live) that splits the runtime in two."
---
...
| **Plugin shared state** (e.g. poll `votes`) | the plugin, via `pluginState()` | Yjs `plugin:<id>` map (CRDT), or `plugin:<id>:<instance>` for a named instance ((internal ADR)) | **any** participant writes (incl. read-only viewers); presenter-only actions gated in plugin code | ephemeral (lost on server restart) |
