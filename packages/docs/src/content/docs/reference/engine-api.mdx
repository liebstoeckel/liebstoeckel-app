...
| `Atmosphere` | component | the default backdrop: static gradient blooms + vignette (`still?`, opt-in `drift?` via CSS keyframes, honors `prefers-reduced-motion`) |
| `Present`/`Deck` `backdrop` prop | prop | deck-wide decoration behind the slide transition stack: omit for `Atmosphere`, `null` for a flat background, or a component receiving `still?` (it must render motionless when set: thumbnails, print, capture, presenter previews) |
| `Backdrop` · `BackdropProvider` · `BackdropComponent` | component/type | the resolved backdrop of the current deck (custom views render `<Backdrop still />` themselves) |
...
| `SlideFrame` | component | the padded content frame of a slide (transparent; the deck backdrop renders behind it) |
...
| `LiveProvider` · `useLive` · `LiveContextValue` | component/hook/type | the live context (with `connection`, the live connection's state) |
| `detectLive` · `LiveInfo` | fn/type | is a live server connected? |
| `connectLive` · `LiveConnection` | fn/type | connect to a session over WebSocket; `onStatus` (connected or not) and `onState` (`LiveState`) |
| `LiveState` · `LiveStatus` | types | `connecting`, `connected`, `reconnecting`, `ended`, `outdated` (with the server's message) |
| `LIVE_CLOSE` · `LIVE_PROTOCOL` · `withLiveProtocol` | consts/fn | the live socket's close codes and protocol version |
