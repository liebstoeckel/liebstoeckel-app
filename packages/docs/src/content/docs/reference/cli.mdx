...
`dist/poll-demo.html` ((internal ADR)). By default the build also
...
with `--dir <deck>` (the convention shared by `pack`/`live`/`export` — (internal ADR)).
...
The deck defaults to the **current directory**; pass it as `[dir]` or `--dir <deck>` ((internal ADR)).
...
`--dir <deck>` (the same convention as `build`/`pack`/`export` — (internal ADR)).
...
`<title>` → the deck key ((internal ADR)): the server reads the `<title>` straight from the
