---
title: CLI reference
description: The umbrella liebstoeckel command — new, add, registry, build, eject, pack, live, relay, thumbs, export, skill, login, push, orgs, decks, brand.
---
...
liebstoeckel push                        # edit + re-push → a new VERSION of the same deck
```
...
separate deck. The **title** comes from `--title` → the embedded `<title>` → the folder
name (never "index"); a **cover** is read from the deck's built-in thumbnails. With no
path, `push` looks for `./dist/index.html`.
