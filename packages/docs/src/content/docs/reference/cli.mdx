...
the deck; the plain deck alone is at `/deck`), whose first tool is [live annotations](/guides/dev-mode/) an AI agent can apply to
your slide source while you watch.
...
still lists it with `suppressed: true` and the reason, and warns about opt-out
entries that suppress nothing (an empty reason, or an unknown id).
