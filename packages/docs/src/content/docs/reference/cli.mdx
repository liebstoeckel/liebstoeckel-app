...
Serve a deck for authoring: hot reload plus the in-browser dev drawer, whose
first tool is [live annotations](/guides/dev-mode/) an AI agent can apply to
...
`dev poll` blocks until the browser sends work (up to `--timeout` ms, default
600000), prints one JSON event, and exits; every event carries an
`_instructions` field telling the agent exactly what to do next. Replies are
`done` with a JSON result or `error "reason"`.

The first run adds a small inert loader tag to the deck's `index.html`; it only
activates under `dev` and every `build` strips it, so built decks are unaffected.
...
`dev` ships with the monorepo today and is not yet in the published CLI; the
published command prints a notice until the dev-server package is released.
</Aside>
