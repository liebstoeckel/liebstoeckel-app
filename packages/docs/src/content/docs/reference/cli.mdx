...
The build also embeds a **third-party license notice** computed from the modules
that actually ended up in the bundle (React, Motion, Yjs, the fonts, and the
MPL-2.0 engine). Because minification strips license comments, this restores the
attribution those licenses require — and because it's recomputed every build, it
tracks font and library swaps automatically. Read it with [`licenses`](#licenses).
...
the embedded fonts) — build-time tooling such as the MDX compiler, Tailwind, and the
syntax highlighter run during the build and are **not** redistributed, so they're
excluded.

The two inputs differ in what they can do. A built **`.html`** carries its notices as
rendered text, so the command just prints them (with `--json`, as `{ source, notices }`).
A deck **directory** is recomputed from a fresh bundle, so it yields the structured
report (`{ ok, packages, firstParty, flagged }`) and is the only input that supports
`--check`. As with `build --check`, output is JSON when piped or with `--json`.
