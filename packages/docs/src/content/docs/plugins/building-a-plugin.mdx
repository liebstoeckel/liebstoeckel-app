...
`plugin:<id>:<instance>` ((internal ADR)):
...
A slide is rendered on a fixed **1280×720 canvas** that's scaled to fit and **clipped** (`overflow: hidden`), so the audience view, presenter view, and thumbnails stay pixel-identical ((internal ADR)). The canvas **never scrolls**. That's fine for authored slides — you control how much is on them — but a plugin whose content grows with the audience (a Q&A queue, a chat, a long results list) *will* eventually overflow and get cut off, with no way to scroll to the rest.
...
On touch, an interactive plugin opens in a breakout sheet that already scrolls ((internal ADR)). It's the **inline / desktop** path that has no scroll boundary, so test your plugin with a *lot* of content on a desktop window. A `ScrollArea` fixes both at once — and because it has its own boundary, it stays a single clean scroll inside the breakout too.
</Aside>
