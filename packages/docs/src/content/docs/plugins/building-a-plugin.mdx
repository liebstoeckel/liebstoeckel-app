...
On touch, an interactive plugin opens in a breakout sheet that already scrolls. It's the **inline / desktop** path that has no scroll boundary, so test your plugin with a *lot* of content on a desktop window. A `ScrollArea` fixes both at once, and because it has its own boundary, it stays a single clean scroll inside the breakout too.
</Aside>
