...
The server part runs **once**, on the machine that started [`liebstoeckel live`](/guides/live/) — never in the audience's browsers, and **never on the relay**. With a [relay](/guides/relay/), your local process connects as a privileged "runner" peer and applies the server plugin's effects to the shared doc.
