...
Audience write-scope is **enforced server-side**: the live server and relay drop any audience update that touches state outside the plugin's declared `audienceWrites` allowlist, and rate-limit the rest. The presenter (or runner) may write the whole doc; an audience peer can only touch its allowed keys. The remaining trust assumption is **non-malicious clients** (XSS must still be impossible), since a plugin's *client* code runs in the audience's browser.
</Aside>
