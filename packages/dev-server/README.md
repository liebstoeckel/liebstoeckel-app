...
It serves a shell page at `/` (the annotation sidebar beside the deck, which it frames from a per-session token path; `/deck` redirects there) where you annotate slides directly in the browser (comments, strokes, a screenshot with your marks baked in). The plain deck is served with HMR; a small loader tag in the deck's `index.html` pulls in the bridge script that connects the framed deck to the sidebar. An AI agent picks those annotations up through a long-poll loop (`liebstoeckel dev poll`), applies them to the slide source, and hot reload shows the edit without losing your place in the deck. On boot it also applies any pending scaffold migrations for the deck. Everything is local: the API binds `127.0.0.1` by default and every mutating request needs a per-session token.

Most people drive it through `liebstoeckel dev` in [@liebstoeckel/cli](https://www.npmjs.com/package/@liebstoeckel/cli), which depends on this package. Use it directly only when you want to embed the dev server. Exports: `.` (`startDevServer`, the local backend, the store types), `./protocol` (the host-agnostic `/__dev/*` handler behind a `DevBackend`), `./ui` (the React `DevShell` / `DevSidebar`), `./bridge` (the in-frame half of the shell bridge), `./cli` (the `dev` and `dev poll` commands).
