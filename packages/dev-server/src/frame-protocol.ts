...
// drawer/drawer.ts (frame). Locally the two are same-origin; hosted, the frame
// is a sandboxed opaque origin, so the parent addresses it by window and the
// frame answers to the origin that greeted it. Keep free of bun / node / CLI
// imports: this runs in browsers.
...
export function isDraftPayload(d: unknown): d is DraftPayload {
  ...
  const { strokes, comments, space } = d as Partial<DraftPayload>;
  return (
    space === "stage" &&
    Array.isArray(strokes) &&
    strokes.every((s) => s && Array.isArray((s as StrokeDraft).points) && (s as StrokeDraft).points.every(isPoint)) &&
    Array.isArray(comments) &&
    comments.every((c) => c && typeof (c as CommentDraft).x === "number" && typeof (c as CommentDraft).y === "number" && typeof (c as CommentDraft).text === "string")
  );
}
 ...
 *  answers with init, only then does the frame act on anything. Messages from
 *  any origin other than the one that sent init are dropped. */
...
export function acceptInit(h: Handshake, origin: string, data: unknown): Handshake {
