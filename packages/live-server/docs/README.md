# Live server (`@liebstoeckel/live-server`)
...
```bash
bunx liebstoeckel <deck.html | deck-project-dir>
# or in this repo:
bun run poll:live
```
...
- **`GET /?t=…`** → resolve role; inject `window.__LIEBSTOECKEL_LIVE__ = { ws, token, role, viewer }` into the deck HTML and return it. (Absent token → 403.)
...
1. discovers plugins from the deck's `package.json` deps (keyword `liebstoeckel-plugin` + a `liebstoeckel` field),
2. bundles each server entry with `Bun.build` (`target:"bun"`, self-contained),
3. **base64-embeds** it in an inert `<script type="application/json" data-liebstoeckel-plugins>` manifest in the HTML.
